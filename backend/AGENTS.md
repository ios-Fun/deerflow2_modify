...
- **UserInputEnrichmentMiddleware** *(between 14 and 15)* - Calls the configured `match_for_best` MCP tool for the latest real user input and injects matched entities as a hidden `HumanMessage` into `ModelRequest.messages` through `awrap_model_call`. The injection is request-only: it must never write to `ThreadState.messages` or checkpoints, because persisted framework-authored HumanMessages change first-turn counting in `TitleMiddleware` and pollute visible history. The first result (including an empty result) is cached in a redacted run-context key and reused across later model calls in the same tool loop, so one user turn performs at most one match request.
